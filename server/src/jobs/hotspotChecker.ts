...
import { searchSogou, searchBilibili, searchWeibo, detectAndFetchAccount, searchWeixin, searchRSS } from '../services/chinaSearch.js';
...
export async function runHotspotCheck(io: Server): Promise<void> {
  ...
  for (const keyword of keywords) {
    ...
    try {
      ...
      // 处理结果：统一配额（不再优先 Twitter）
      // 总共处理最多 N 条候选以控制成本与入库量
      let processedCount = 0;
      const TOTAL_QUOTA = 25;

      for (const item of sortedResults) {
        // 检查配额
        if (processedCount >= TOTAL_QUOTA) break;
        try {
          // 检查是否已存在
